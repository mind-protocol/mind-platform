...
export async function DELETE(
  ...
  try {
    const res = await manemusFetch(
      `/api/feed/${encodeURIComponent(profileId)}/${encodeURIComponent(postId)}/comments/${encodeURIComponent(commentId)}`,
      { method: 'DELETE', headers: { Authorization: `Bearer ${apiKey}`, 'X-User-Id': auth.user_id } }
    );
