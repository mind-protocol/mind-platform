...
function Door({ icon, identity, message }: DoorProps) {
  return (
    <div className="p-8 rounded-xl border border-zinc-800 hover:border-zinc-700 bg-zinc-900/30 hover:bg-zinc-900/50 transition-all duration-300">
      <div className="text-3xl mb-4" role="img" aria-label={identity}>{icon}</div>
...
export function WhoThisIsFor() {
  ...
  const doors: DoorProps[] = [
    { icon: '\u{1F56F}\uFE0F', identity: t('door1Identity'), message: t('door1Message') },
    { icon: '\u{1F916}', identity: t('door2Identity'), message: t('door2Message') },
    { icon: '\u{1F528}', identity: t('door3Identity'), message: t('door3Message') },
    { icon: '\u{1F3DB}\uFE0F', identity: t('door4Identity'), message: t('door4Message') },
  ];
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {doors.map((door, index) => (
            <FadeInSection key={door.identity} delay={index * 100}>
              <Door {...door} />
            </FadeInSection>
          ))}
        </div>
