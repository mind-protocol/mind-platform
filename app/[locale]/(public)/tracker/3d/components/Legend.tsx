...
export default function Legend() {
  ...
  return (
    <div className="bg-zinc-900/80 backdrop-blur-sm border border-zinc-800 rounded-lg p-3 space-y-1.5">
      <div className="text-[10px] uppercase tracking-wider text-zinc-600 mb-2">{t('legendSubstances')}</div>
      ...
      <div className="text-[10px] uppercase tracking-wider text-zinc-600 mb-1">{t('legendBiometrics')}</div>
      <div className="flex items-center gap-2 text-xs">
        <div className="w-2.5 h-1 rounded bg-red-500/60" />
        <span className="text-zinc-500">{t('legendStress')}</span>
      </div>
      <div className="flex items-center gap-2 text-xs">
        <div className="w-2.5 h-1 rounded bg-cyan-500/60" />
        <span className="text-zinc-500">{t('legendBodyBattery')}</span>
      </div>
    </div>
  );
}
