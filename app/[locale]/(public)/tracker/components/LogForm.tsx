...
import MindAnalysisCard, { type MindAnalysisData } from './MindAnalysisCard';
...
export default function LogForm({ onLogged, filter }: { onLogged: () => void; filter?: string[] }) {
  ...
  // MIND analysis state
  const [mindAnalysis, setMindAnalysis] = useState<MindAnalysisData | null>(null);
  ...
  return (
    ...
    >
      ...
      {/* MIND Analysis Card (extracted component with React.memo) */}
      <MindAnalysisCard
        analysis={mindAnalysis}
        analyzing={mindAnalyzing}
        collapsed={analysisCollapsed}
        onToggleCollapsed={() => setAnalysisCollapsed(!analysisCollapsed)}
      />
