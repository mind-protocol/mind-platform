...
describe('GET /api/house', () => {
  ...
  it('returns house state from backend', async () => {
    mockManemusFetchJson.mockResolvedValueOnce({
      data: {
        ts: '2026-02-27T10:00:00Z',
        rooms: [{ name: 'lobby', occupants: 3 }],
        hallway: [],
        neon: {},
        ceiling: {},
        streets: { citizen_count: 10, garmin_linked: 5, recent: [] },
        meta: { room_count: 1, hallway_events: 0, has_neon: false, has_music: true },
      },
    });

    const { GET } = await import('@/app/api/house/route');
    const response = await GET();
    expect(response.status).toBe(200);
    const data = await response.json();
    expect(data.rooms).toHaveLength(1);
    expect(data.streets.citizen_count).toBe(10);
    expect(data.meta.has_music).toBe(true);
  });
  ...
  it('returns offline fallback when backend is unreachable', async () => {
    ...
    expect(data.streets.citizen_count).toBe(0);
  });
});
...
describe('GET /api/graph', () => {
  ...
  it('returns graph data from backend', async () => {
    mockManemusFetchJson.mockResolvedValueOnce({
      data: { nodes: [{ id: 'n1' }], edges: [{ from: 'n1', to: 'n2' }] },
      status: 200,
    });

    const { GET } = await import('@/app/api/graph/route');
    const req = makeGetRequest('http://localhost/api/graph');
  ...
  it('passes query params to backend', async () => {
    ...
    const { GET } = await import('@/app/api/graph/route');
    const req = makeGetRequest('http://localhost/api/graph?depth=2&root=n1');
    const response = await GET(req);
    expect(response.status).toBe(200);
...
describe('GET /api/graph/stats', () => {
  ...
  it('returns graph stats from backend', async () => {
    mockManemusFetchJson.mockResolvedValueOnce({
      data: { node_count: 42, edge_count: 100 },
      status: 200,
    });

    const { GET } = await import('@/app/api/graph/stats/route');
    const response = await GET();
    expect(response.status).toBe(200);
    const data = await response.json();
    expect(data.node_count).toBe(42);
    expect(data.edge_count).toBe(100);

    expect(mockManemusFetchJson).toHaveBeenCalledWith('/api/graph/stats');
  });
...
describe('GET /api/graph/search', () => {
  ...
  it('forwards search query to backend', async () => {
    mockManemusFetchJson.mockResolvedValueOnce({
      data: { results: [{ id: 'n1', name: 'Alice' }] },
      status: 200,
    });

    const { GET } = await import('@/app/api/graph/search/route');
    const req = makeGetRequest('http://localhost/api/graph/search?q=alice');
    const response = await GET(req);
    expect(response.status).toBe(200);
    const data = await response.json();
  ...
  it('returns 502 when backend is unreachable', async () => {
    ...
    const { GET } = await import('@/app/api/graph/search/route');
    const req = makeGetRequest('http://localhost/api/graph/search?q=test');
    const response = await GET(req);
    expect(response.status).toBe(502);
  });
});
...
describe('GET /api/graph/node/[id]', () => {
  ...
  it('returns node data by id', async () => {
    mockManemusFetchJson.mockResolvedValueOnce({
      data: { id: 'n1', name: 'Alice', type: 'CITIZEN' },
      status: 200,
    });

    const { GET } = await import('@/app/api/graph/node/[id]/route');
    const req = new Request('http://localhost/api/graph/node/n1');
    const response = await GET(req, { params: Promise.resolve({ id: 'n1' }) });
    expect(response.status).toBe(200);
    const data = await response.json();
    expect(data.id).toBe('n1');
    expect(data.name).toBe('Alice');

    expect(mockManemusFetchJson).toHaveBeenCalledWith('/api/graph/node/n1');
  });
